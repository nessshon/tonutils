"""
Example: Verifying signed data using `verify_sign_data` from tonutils.

The TonConnect client provides the signed payload (see `EXAMPLE_DATA` below),
which must be collected on the client and sent to your backend for verification.

By default, `verify_sign_data` will try to extract the public key from the provided `state_init`.
If that fails (e.g. for custom wallets), you can supply a fallback resolver via
`get_wallet_pubkey(address: str) -> bytes`.
"""
from tonutils.tonconnect.models import CheckSignDataRequestDto
from tonutils.tonconnect.utils.verifiers import verify_sign_data

# This is a sample payload structure collected from the TonConnect client (DApp frontend)
EXAMPLE_DATA = {
...
async def main() -> None:
    payload = CheckSignDataRequestDto.from_dict(EXAMPLE_DATA)

    # Supported payload types:
    # - text: for plain text messages
    # - binary: for arbitrary binary data
    # - cell: for signed TON cells (e.g. TL-B structures)

    # `verify_sign_data` automatically:
    ...
    is_valid = await verify_sign_data(
        payload=payload,
